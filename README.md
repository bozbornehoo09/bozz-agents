...
Codex discovers skills under `.agents/skills/`. This repo's `.agents/skills/` is
**generated by CI** from `skills/` (see below), so a clone already carries a
Codex-ready tree — point Codex at it (repo, user, or admin scope). Details in
